...
namespace DelayedStop
{
	...
	public class DelayedStop : TerrariaPlugin
	{
		...
		public static DateTime ds = DateTime.UtcNow;
		...
		#region get messages
		public static string getTimeLeftMsg()
		{
			string msg = getConfig.TimeLeft_Message;

			if (msg.Contains("@time-left@"))
				msg = msg.Replace("@time-left@", dispTimeLeft());

			return msg;
		}

		public static string getPausedMsg()
		{
			string msg = getConfig.ShutdownPaused_Message;
		...
		public static string getResumedMsg()
		{
			string msg = getConfig.ShutdownResumed_Message;
		...
		{
			if ((DateTime.UtcNow - ds).TotalMilliseconds >= 1000)
			{
				ds = DateTime.UtcNow;
				if (shutdowninprogress && !shutdownpaused && noconferr)
				{
					...
					foreach (string timeint in strNotifiers)
					{
						...
						if (timeleft == numsecs)
							TShock.Utils.Broadcast(getConfig.ChatPrefix + " " + getTimeLeftMsg() + shutdownreason, getConfig.ColorR, getConfig.ColorG, getConfig.ColorB);
					}
					timeleft--;
					if (timeleft == 0)
		...
		public static void dstop(CommandArgs args)
		{
			...
			if (subcmd.ToLower() == "pause" || subcmd.ToLower() == "wait")
			{
				...
				{
					shutdownpaused = true;
					TShock.Utils.Broadcast(getConfig.ChatPrefix + " " + getPausedMsg(), getConfig.ColorR, getConfig.ColorG, getConfig.ColorB);
				}
				else
				{
			...
			else if (subcmd.ToLower() == "resume" || subcmd.ToLower() == "go" || subcmd.ToLower() == "continue")
			{
				if (shutdownpaused == true && shutdowninprogress == true)
				{
					...
					TShock.Utils.Broadcast(getConfig.ChatPrefix + " " + getResumedMsg(), getConfig.ColorR, getConfig.ColorG, getConfig.ColorB);
				}
				else
				{
					args.Player.SendMessage("Error: No Delayed Shutdown in progress!", Color.Red);
		...
		{
			Color Colour = new Color(getConfig.ColorR, getConfig.ColorG, getConfig.ColorB);
			args.Player.SendMessage("Shutdown in progress: " + shutdowninprogress, Colour);
			args.Player.SendMessage("Shutdown Paused: " + shutdownpaused, Colour);
			args.Player.SendMessage("Time left till shutdown: " + dispTimeLeft(), Colour);
			args.Player.SendMessage("Reason for shutdown:" + shutdownreason, Colour);
		}
	}
}
